...
import { tPriceRow } from '../contentI18n';
...
export default function Prices() {
  ...
  const rows = prices.map(p => tPriceRow(p, lang));

  // `highlight` marks the format currently in demand — it draws the "popular"
  // pill in the legend and tints that column of the table. Individual study is
  // what most students are taking now, so it carries the marker.
  const formats = [
    { key: 'group', label: t('group_label'), icon: '👥', desc: t('group_desc'), highlight: false },
    { key: 'mini_group', label: t('mini_group_label'), icon: '👫', desc: t('mini_desc'), highlight: false },
    { key: 'individual', label: t('individual_label'), icon: '👤', desc: t('indiv_desc'), highlight: true },
  ];
  ...
  return (
    <div className="prices-page">
      ...
      <div className="container prices-body">
        ...
        {/* Price Table */}
        {loading ? <div className="page-loading" style={{minHeight:200}}>{t('loading')}</div> : (
